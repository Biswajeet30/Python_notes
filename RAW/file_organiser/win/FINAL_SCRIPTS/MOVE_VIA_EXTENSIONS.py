...
# ### changeable  source folder as per operations
source_folder = r"E:\political bhagoda"
### changeable  target folder as per operations
target_folder = r"E:\political bhagoda\pics_txts"
new_directory = r"E:\political bhagoda\pics_txts\logs"
report_transferred_files_logFILE_path = r"E:\political bhagoda\pics_txts\logs\transferred_files_log.txt"
report_remaining_files_logFILE_path = r"E:\political bhagoda\pics_txts\logs\remaining_files_log.txt"
...
def find_and_sort_videos_by_filename(folder_path):
    ...
    for item in os.listdir(folder_path):
        ...
        has_valid_extension = item.lower().endswith(('.apng', '.avif', '.gif', '.jpg', '.jpeg', '.jfif', '.pjpeg', 
                                                      '.pjp', '.png', '.svg', '.webp', '.tif', '.tiff', '.psd', '.raw', 
                                                      '.arw', '.cr2', '.nrw', '.k25', '.bmp', '.dib', '.heif', '.heic', 
                                                      '.ind', '.indd', '.indt', '.ai', '.eps', '.doc', '.docx', 
                                                      '.html', '.htm', '.odt', '.pdf', '.xls', '.xlsx', '.ods', 
                                                      '.ppt', '.pptx', '.txt', '.log', '.csv', '.xml', '.json', '.yaml', 
                                                      '.ini', '.cfg', '.conf', '.config', '.md', 
                                                      '.markdown','.exe', '.com', 
                                                      '.bat', '.cmd', '.msi', '.dll', '.sys', '.scr', 
                                                      '.vbs', '.vbe', '.js', '.ps1', 
                                                      '.psm1', '.psd1', '.psc1', '.msp', '.mst', 
                                                      '.msp', '.msi', '.msp', '.tar'))

        # Check if the item is a file and has a valid video file extension
        if is_file and has_valid_extension:
            other_ext_files.append(item)

    # Sort the video files by their filenames
    sorted_files = sorted(other_ext_files)
    return sorted_files
...
def move_files(source_folder, target_folder, log_file):
    """
    ...
    for item in os.listdir(source_folder):
        ...
        has_valid_extension = item.lower().endswith(('.apng', '.avif', '.gif', '.jpg', '.jpeg', '.jfif', '.pjpeg', 
                                                      ...
                                                      '.psm1', '.psd1', '.psc1', '.msp', '.mst', 
                                                      '.msp', '.msi', '.msp', '.tar'))
        ### changeable  string as per operations
        # = "India" in item  or "Iyer Mitra" or "Chavda "
    ...
    for other_files in other_ext_files:
        source_path = os.path.join(source_folder, other_files)
        destination_path = os.path.join(target_folder, other_files)
        ...
        while retries < max_retries:
            try:
                ...
                logging.warning(f"File {other_files} is in use by another process. Retrying...")
                retries += 1
            ...
            logging.error(f"\n\nFailed to move {other_files}: File is still in use after {max_retries} retries")
...
def main():
    ...
    with open(os.path.join(new_directory, 'FIle_Transfer_VIA_NAME_info.txt'), 'a', encoding='utf-8') as log_file:
        ...
        move_files(source_folder, target_folder, log_file)
